...
class Debitos(db.Model):
    ...
    def atualizar_produtos(self, id, nome, tipoPagamento, data, status, descriçao):
        try:
            db.session.query(Debitos).filter(Debitos.id == id).update(
                {
                    "nome": nome,
    ...
    def deletar_debito(delf, id):
        try:
            db.session.query(Debitos).filter(Debitos.id == id).delete()
            db.session.commit()
        except Exception as e:
            print(e)
